...
public:
    ...
    SingleLinkedList(std::initializer_list<Type> values) {
        ...
        CopyList(values.begin(), values.end());
    }
...
private:
    ...
    void CopyList(It other_begin, It other_end) {
        if (other_begin == other_end) {
            return;
        }
        SingleLinkedList tmp;
        tmp.PushFront(*other_begin);
        auto it_buf = tmp.begin();
        for (auto it = ++other_begin; it != other_end; ++it) {
            it_buf = tmp.InsertAfter(it_buf, *it);
        }
        swap(tmp);
    }
};
